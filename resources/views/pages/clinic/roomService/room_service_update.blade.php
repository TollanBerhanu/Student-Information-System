...
@section('content')
    ...
    <div class="card card-custom">
        <div class="card-header">
            <h3 class="card-title">
                Clinic Room Service Update Form
            </h3>
        </div>
        <!--begin::Form-->
        <form method="POST" enctype="multipart/form-data" action="{{ route("room_ServiceUpdateHandle") }}">
            @csrf
            <div class="card-body">
                ...
                <input type="hidden" value="{{$room_service['id']}}" name="id">
                <div class="form-group row">
                    <label class="col-2 col-form-label">Name</label>
                    <div class="col-10">
                        <input class="form-control" type="text" value="{{ old('name') ? old('name') : $room_service['name'] }}" name="name"/>
                    </div>
                </div>
                <div class="form-group">
                    <label>Description</label>
                    <textarea class="form-control form-control-solid" rows="3" name="description">{{old('description') ? old('description') : $room_service['description']}}</textarea>
                </div>
                <div class="form-group">
                    ...
                    <select class="form-control form-control-solid" name="room_id">
                        ...
                        @foreach($rooms as $room)
                            <option value="{{$room['id']}}" {{old('room_id') ? (old('room_id') == $room['id'] ? "selected" : "") : (($room_service['room_id'] == $room['id'] ? "selected" : "")) }}>{{$room['name']}}</option>
                        @endforeach
                    </select>
