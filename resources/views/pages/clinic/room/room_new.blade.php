...
@section('content')
    ...
    <div class="card card-custom">
        <div class="card-header">
            <h3 class="card-title">
                Clinic Room Registration Form
            </h3>
        </div>
        <!--begin::Form-->
        <form method="POST" enctype="multipart/form-data" action="{{ route("roomRegisterHandle") }}">
            @csrf
            <div class="card-body">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control form-control-solid" name="clinic_id">
                        ...
                        @foreach($clinics as $clinic)
                            <option value="{{$clinic['id']}}" {{old('clinic_id') ? (old('clinic_id') == $clinic['id'] ? "selected" : "") : "" }}>{{$clinic['name']}}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
                    ...
                    <select class="form-control form-control-solid" name="room_type_id">
                        ...
                        @foreach($room_types as $room_type)
                            <option value="{{$room_type['id']}}" {{old('room_type_id') ? (old('room_type_id') == $room_type['id'] ? "selected" : "") : "" }}>{{$room_type['name']}}</option>
                        @endforeach
                    </select>
                </div>
            </div>
